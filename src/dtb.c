...
int dtb_fdt_prop_parse(uint8_t** curr_struct_offset, const uint8_t* string_base)
{
...
void dtb_parser(uint64_t DTB_BASE)
{
    ...
    while (dtb_fdt_begin_parse(&curr_struct_offset) == 0) {
        while (dtb_fdt_prop_parse(&curr_struct_offset, string_base) == 0) {
